...
serve(async (req) => {
  ...
  try {
    ...
    if (!clusterId || !region) {
      return new Response(
        JSON.stringify({ error: "Missing clusterId or region" }),
        {
          status: 400,
    ...
    if (!authRes.ok) {
      const oauthError = new Error(
        `Azure OAuth failed: ${await authRes.text()}`,
      );
    ...
    const vmResult = await deleteResource(
    ...
    await delay(3000);
    const nicResult = await deleteResource(
    ...
    const pipResult = await deleteResource(
    ...
    const nsgResult = await deleteResource(
      "Microsoft.Network/networkSecurityGroups",
      nsgName,
      "2023-09-01",
    );
    const vnetResult = await deleteResource(
    ...
    // 6. Deployment record
    const deployResult = await deleteResource(
